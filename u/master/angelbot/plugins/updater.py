...
from AngelBot.helpers import runner
from . import *
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "✔︎ Successfully updated AngelBot!\n\nBot is restarting please wait for a minute."
    )
    args = [sys.executable, "-m", "AngelBot"]
    os.execle(sys.executable, *args, os.environ)
    return


@bot.on(angel_cmd(outgoing=True, pattern=r"update(| now)$"))
...
async def upstream(event):
    ...
    if force_update:
        await event.edit(
            "`✔️ Force-Updating ⋆𝗔𝘆𝘂𝘀𝗵𝗮™⋆. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your  ⋆𝗔𝘆𝘂𝘀𝗵𝗮™⋆ Is UpToDate**\n\n**Version :**  __{angel_ver}__\n**Oɯɳҽɾ :**  {angel_mention}")
    else:
        await event.edit("**Please set up**  `HEROKU_API_KEY`  **from heroku to update!**")
    return


@bot.on(angel_cmd(outgoing=True, pattern=r"update build$"))
...
async def upstream(event):
    ...
    off_repo = "https://github.com/TEAM-angel/angelBot"
    os.chdir("/app")
    git_angel = f"rm -rf .git"
    try:
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit(f"**⋆𝗔𝘆𝘂𝘀𝗵𝗮™⋆ Docker Build In Progress... Type** `{hl}ping`  **after 5 mins to check if Bot is working!**")
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your ⋆𝗔𝘆𝘂𝘀𝗵𝗮™⋆. Basically if you restart dyno it will go back to previous deploy."
).add_command(
  "update build", None, "Hard-Update Your ⋆𝗔𝘆𝘂𝘀𝗵𝗮™⋆. This won't take you back to your previous deploy. This will be triggered even if there is no changelog."
).add()
