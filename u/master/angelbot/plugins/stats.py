...
@bot.on(angel_cmd(pattern="stats$"))
...
async def stats(
    ...
    if event.fwd_from:
        return
    angel = await edit_or_reply(event, "`Collecting stats...`")
    ...
    response += (
        f"📌 𝙵𝚁𝙾𝙼 𝚃𝙷𝙴 𝙳𝙰𝚃𝙰𝙱𝙰𝚂𝙴 𝙾𝙵 :- {angel_channel}" 
    )
    await angel.edit(response)
...
def inline_mention(user):
    full_name = user_full_name(user) or "angel"
