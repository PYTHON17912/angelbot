...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    bot = "@asciiart_bot"
    angelkrish = await eor(event, "Wait making ASCII...🤓🔥🔥")
    async with event.client.conversation(bot) as conv:
        try:
            ...
            await angelkrish.edit("User Blocked!! Please Unblock @asciiart_bot and try again...")
            return
...
async def _(event):
    ...
    if not reply_message.media:
        ...
        return
    bot = "@lines50bot"
    angelkrish = await eor(event, "`Processing...`")
    async with event.client.conversation(bot) as conv:
        try:
            first = await conv.send_message("/start")
            response = await conv.get_response()
            ...
            await angelkrish.edit("User Blocked!! Please Unblock @Lines50Bot and try again...")
            return
        await angelkrish.delete()
        final = await event.client.send_file(
            event.chat_id,
