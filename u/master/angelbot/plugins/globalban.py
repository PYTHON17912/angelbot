...
import asyncio
from angelbot.sql.gban_sql import is_gbanned, gbaner, ungbaner, all_gbanned
from angelbot.sql import gmute_sql as gsql
from . import *


@bot.on(angel_cmd(pattern=r"gban ?(.*)"))
@bot.on(sudo_cmd(pattern=r"gban ?(.*)", allow_sudo=True))
async def _(event):
    angel = await eor(event, "`Gbanning...`")
    reason = ""
    if event.reply_to_msg_id:
        ...
        return await eod(angel, "**To gban a user i need a userid or reply to his/her message!!**")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == d3krish:
        return await eod(angel, "🥴 **Nashe me hai kya lawde ‽**")
    if str(userid) in DEVLIST:
        return await eod(angel, "😑 **GBan my creator ?¿ Really‽**")
    ...
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                ...
                pass
    gbaner(userid)
    gmsg = f"🥴 𝚃𝙷𝙸𝚂 𝚄𝚂𝙴𝚁 𝙶𝙻𝙾𝙱𝙰𝙻 𝙱𝙰𝙽  【[{name}](tg://user?id={userid})】 **𝙱𝚈 :** 『{angel_mention}』\n\n📍 𝙰𝚍𝚍𝚎𝚍 𝚝𝚘 𝙶𝚋𝚊𝚗 𝚆𝚊𝚝𝚌𝚑!!\n** 𝚃𝚘𝚝𝚊𝚕 𝙲𝚑𝚊𝚝𝚜 :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**𝚁𝚎𝚊𝚜𝚘𝚗 ➳➠ :**  `{reason}`"
    ogmsg = f"[{name}](tg://user?id={userid}) **𝙸𝚜 𝚗𝚘𝚠 𝙶𝙱𝚊𝚗𝚗𝚎𝚍 𝚋𝚢** {angel_mention} **in**  `{chats}`  **𝙲𝚑𝚊𝚝𝚜!! 😏**\n\n**📍 𝙰𝚕𝚜𝚘 𝙰𝚍𝚍𝚎𝚍 𝚝𝚘 𝙶𝚋𝚊𝚗 𝚆𝚊𝚝𝚌𝚑!!**"
    if reason != "":
        ogmsg += f"\n**𝚁𝚎𝚊𝚜𝚘𝚗 ➳➠ :**  `{reason}`"
    if Config.ABUSE == "ON":
        await bot.send_file(event.chat_id, cjb, caption=gmsg)
    else:
        await angel.edit(ogmsg)


@bot.on(angel_cmd(pattern=r"ungban ?(.*)"))
@bot.on(sudo_cmd(pattern=r"ungban ?(.*)", allow_sudo=True))
async def _(event):
    angel = await eor(event, "`Ungban in progress...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        ...
        return await eod(angel, "`Reply to a user or give their userid... `")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if not is_gbanned(userid):
        return await eod(angel, "`User is not gbanned.`")
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
...
@bot.on(angel_cmd(pattern="listgban$"))
...
@bot.on(angel_cmd(pattern=r"gkick ?(.*)"))
@bot.on(sudo_cmd(pattern=r"gkick ?(.*)", allow_sudo=True))
async def gkick(event):
    angel = await eor(event, "`Kicking globally...`")
    if event.reply_to_msg_id:
        userid = (await event.get_reply_message()).sender_id
    elif event.pattern_match.group(1):
        userid = await get_user_id(event.pattern_match.group(1))
        ...
        return await eod(angel, "`Reply to some msg or add their id.`")
    name = (await event.client.get_entity(userid)).first_name
    chats = 0
    if userid == d3krish:
        return await eod(angel, "**🥴 Nashe me hai kya lawde!!**")
    if str(userid) in DEVLIST:
        return await eod(angel, "**😪 I'm not going to gkick my developer!!**")
    ...
    if Config.ABUSE == "ON":
        ...
        await angel.edit(gkmsg)


@bot.on(angel_cmd(pattern=r"gmute ?(\d+)?"))
...
@bot.on(angel_cmd(outgoing=True, pattern=r"ungmute ?(\d+)?"))
