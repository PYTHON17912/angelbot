...
from angelbot.sql import antiflood_sql as sq
from . import *
...
@angelbot.on(admin_cmd(incoming=True))
async def _(event):
    if event.fwd_from:
        return
    if not CHAT_FLOOD:
...
@angelbot.on(admin_cmd(pattern="setflood(?: |$)(.*)"))
@angelbot.on(sudo_cmd(pattern="setflood(?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
