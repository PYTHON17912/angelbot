...
@angelbot.on(events.NewMessage(outgoing=True))  # pylint:disable=E0602
async def set_not_afk(event):
    ...
    if "#" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        angelbot = await event.client.send_message(
            event.chat_id,
        ...
        try:
            ...
            await bot.send_message(  # pylint:disable=E0602
                ...
                + f"Ask in {angel_grp} to get d3vlp!",
                reply_to=event.message.id,
                link_preview=False,
                silent=True,
            )
        await asyncio.sleep(5)
        await angelbot.delete()
...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        msg = await event.reply(message_to_reply, file=angelpic)
...
@angelbot.on(angel_cmd(pattern=r"afk (.*)", outgoing=True))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    angelkrishop = await event.get_reply_message()
    global USER_AFK  # pylint:disable=E0602
    global afk_time  # pylint:disable=E0602
    global last_afk_message  # pylint:disable=E0602
    global afk_start
    ...
    angelpic = await event.client.download_media(angelkrishop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {angelpic}"  # pylint:disable=E0602
        if reason:
            await bot.send_message(
                event.chat_id, f"**I'm going afk🚶** \n\n**Because :** {reason}", file=angelpic
            )
        else:
            await bot.send_message(
                event.chat_id, f"**I am Going afk!**🚶", file=angelpic)
        ...
        try:
            if reason:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**\nReason  `{reason}`",file=angelpic
                 )
            else:
                await bot.send_message(
                  Config.LOGGER_ID,
                  f"#AFKTRUE \nAFK mode = **True**",file=angelpic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
...
@bot.on(angel_cmd(pattern=r"night ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_night
