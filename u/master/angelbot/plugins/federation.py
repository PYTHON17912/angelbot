...
@bot.on(angel_cmd(pattern="newfed ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angel_input = event.pattern_match.group(1)
    chat = "@MissRose_Bot"
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {angel_input}")
...
@bot.on(angel_cmd(pattern="renamefed ?(.*)"))
...
async def _(event):
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {angel_input}")
              response = await response 
          except YouBlockedUserError: 
...
@bot.on(angel_cmd(pattern="fstat ?(.*)"))
@bot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angel = await eor(event, "`Collecting fstat....`")
    thumb = angel_logo
    ...
    if lavde == "":
        ...
        async with bot.conversation(fbot) as conv:
            try:
                ...
                await angel.edit("`Please Unblock` @MissRose_Bot")


@bot.on(angel_cmd(pattern="fedinfo ?(.*)"))
@bot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angel = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with bot.conversation(fbot) as conv:
        try:
            ...
            await angel.edit(massive.text + "\n\n**🎖️ʟɛɢɛռɖǟʀʏ ǟʄ ᴀɴɢᴇʟʙᴏᴛ🎖️**")
        except YouBlockedUserError:
            await angel.edit("`Please Unblock` @MissRose_Bot")
