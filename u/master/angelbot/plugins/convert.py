...
@bot.on(angel_cmd(pattern="stoi$"))
@bot.on(sudo_cmd(pattern="stoi$", allow_sudo=True))
async def _(angel):
    if angel.fwd_from:
        return
    reply_to_id = angel.message.id
    if angel.reply_to_msg_id:
        reply_to_id = angel.reply_to_msg_id
    event = await eor(angel, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await angel.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await angel.client.send_file(
                event.chat_id,
...
@bot.on(angel_cmd(pattern="gif$"))
@bot.on(sudo_cmd(pattern="gif$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angelreply = await event.get_reply_message()
    if not angelreply or not angelreply.media or not angelreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if angelreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    angelevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with bot.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await bot.send_file(chat, angelreply.media)
            response = await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await angelevent.edit("`This file is not supported`")
            angelresponse = response if response.media else await conv.get_response()
            await bot.send_read_acknowledge(conv.chat_id)
            angelfile = Path(await event.client.download_media(angelresponse, "./temp/"))
            angelgif = Path(await unzip(angelfile))
            angelkrish = await bot.send_file(
                event.chat_id,
                angelgif,
            ...
            await bot(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=angelkrish.media.document.id,
                        access_hash=angelkrish.media.document.access_hash,
                        file_reference=angelkrish.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            await angelevent.delete()
            for files in (angelgif, angelfile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await angelevent.edit("Unblock @tgstogifbot")
            return


@bot.on(angel_cmd(pattern="nfc ?(.*)"))
