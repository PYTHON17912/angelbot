...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    angelevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(angelevent, "`Please unblock `@DrWebBot `and try again`")
            return
        if response.text.startswith("Forward"):
            await eod(angelevent,
...
@bot.on(angel_cmd(pattern=r"decode$", outgoing=True))
...
@bot.on(angel_cmd(pattern="barcode ?(.*)"))
@bot.on(sudo_cmd(pattern="barcode ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angelevent = await eor(event, "...")
    ...
    try:
        ...
        await angelevent.edit(str(e))
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await angelevent.edit("Created BarCode in {} seconds🤓".format(ms))
    await asyncio.sleep(5)
    await angelevent.delete()


@bot.on(angel_cmd(pattern=r"makeqr(?: |$)([\s\S]*)", outgoing=True))
...
async def _(event):
    ...
    start = datetime.datetime.now()
    input_str = event.pattern_match.group(1)
    angel = await eor(event, "Processing...")
    ...
    if len(input_sgra) == 3:
        ...
        await angel.edit(str(a))
    else:
        await eod(angel, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds


@bot.on(angel_cmd(pattern="currency (.*)"))
...
@bot.on(angel_cmd(pattern="currencies$"))
...
@bot.on(angel_cmd(pattern="color (.*)"))
...
@bot.on(angel_cmd(pattern="xkcd ?(.*)"))
@bot.on(sudo_cmd(pattern="xkcd ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angelevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
Year: {}""".format(
        ...
        await angelevent.edit(output_str, link_preview=True)
    else:
        await eod(angelevent, "xkcd n.{} not found!".format(xkcd_id))

@bot.on(angel_cmd(pattern="dns (.*)", outgoing=True))
...
@bot.on(angel_cmd(pattern="url (.*)", outgoing=True))
...
@bot.on(angel_cmd(pattern="unshort (.*)", outgoing=True))
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for angelBot", ".currency 10 usd inr"
).add_command(
