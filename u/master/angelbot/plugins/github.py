...
@bot.on(angel_cmd(pattern=r"commit"))
...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        await eod(event, "`Please ADD Proper Github Repo Name of angelBot`")
        return
    angelbot = await eor(event, "Processing ...")
    ...
    try:
        ...
        await angelbot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, angelbot)


async def git_commit(file_name, angelbot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            ...
            create_file = False
    file_name = "angelbot/plugins/" + file_name
    if create_file == True:
        ...
        try:
            ...
            await eod(angelbot, "Cannot Upload Plugin")
    else:
        return await eod(angelbot, "`Committed Suicide`")


@bot.on(angel_cmd(pattern="github (.*)", outgoing=True))
