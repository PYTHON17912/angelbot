...
async def get_users(event):
    ...
    if not sender.id == me.id:
        angel = await edit_or_reply(event, "`processing...`")
    else:
        angel = await edit_or_reply(event, "`processing...`")
    angelkrish = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await angel.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await angel.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(angelkrish.full_chat.id):
        try:
