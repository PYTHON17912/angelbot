...
from angelbot.sql.waifu_sql import is_harem, add_grp, rm_grp, get_all_grp
from . import *
...
@bot.on(angel_cmd(pattern="pt ?(.*)"))
@bot.on(sudo_cmd(pattern="pt ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angel = await eor(event, "Hmm..")
...
@bot.on(angel_cmd(pattern="adwaifu ?(.*)"))
...
@bot.on(angel_cmd(pattern="rmwaifu ?(.*)"))
