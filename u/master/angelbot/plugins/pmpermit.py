...
from . import *
from angelbot.sql import pmpermit_sql as pm_sql
...
angel_ZERO = "Go get some sleep retard. \n\n**Blocked !!**"
angel_FIRST = (
    "**Hello, This is ᗪ3ᏉᎥᏝᏰᎧᏖ𝖴𝗅𝗍𝗋𝖺 𝖯𝗋𝗂𝗏𝖺𝗍𝖾 𝖲𝖾𝖼𝗎𝗋𝗂𝗍𝗒 𝖯𝗋𝗈𝗍𝗈𝖼𝗈𝗅⚠️**"
    "{} 𝗂𝗌 𝖼𝗎𝗋𝗋𝖾𝗇𝗍𝗅𝗒 𝗎𝗇𝖺𝗏𝖺𝗂𝗅𝖺𝖻𝗅𝖾.\nThis 𝗂𝗌 𝖺𝗇 𝖺𝗎𝗍𝗈𝗆𝖺𝗍𝖾𝖽 𝗆𝖾𝗌𝗌𝖺𝗀𝖾.\n\n"
    "{}\n\n**𝖯𝗅𝖾𝖺𝗌𝖾 𝖢𝗁𝗈𝗈𝗌𝖾 𝖶𝗁𝗒 𝖸𝗈𝗎 𝖠𝗋𝖾 𝖨𝗇𝖻𝗈𝗑 👇!!**".format(angel_mention, CSTM_PMP)
)

@bot.on(angel_cmd(pattern="block$"))
...
if PM_ON_OFF != "DISABLE":
    ...
    @bot.on(angel_cmd(pattern="(a|approve|allow)$"))
    ...
    @bot.on(angel_cmd(pattern="(da|disapprove|disallow)$"))
    ...
    @bot.on(angel_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
    ...
    async def do_pm_permit_action(chat_ids, event):
        ...
        if PM_WARNS[chat_ids] == Config.MAX_SPAM:
            r = await event.reply(angel_ZERO)
            await asyncio.sleep(3)
