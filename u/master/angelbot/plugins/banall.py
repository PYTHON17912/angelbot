...
@angelbot.on(angel_cmd(pattern=r"kickall ?(.*)"))
@angelbot.on(sudo_cmd(pattern=r"kickall ?(.*)", allow_sudo=True))
async def _(event):
    ...
    angel = await eor(event, "**Bleck Magik Started...**")
...
@angelbot.on(angel_cmd(pattern=r"banall ?(.*)"))
@angelbot.on(sudo_cmd(pattern=r"banall ?(.*)", allow_sudo=True))
async def _(event):
    result = await event.client(
        functions.channels.GetParticipantRequest(event.chat_id, event.client.uid)
    )
    ...
    angel = await eor(event, "**Bleck Magik Begins..**")
    admins = await event.client.get_participants(
        event.chat_id, filter=ChannelParticipantsAdmins
    )
    admins_id = [i.id for i in admins]
...
@angelbot.on(angel_cmd(pattern=r"unbanall ?(.*)"))
@angelbot.on(sudo_cmd(pattern=r"unbanall ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@angelbot.on(angel_cmd(pattern="ikuck ?(.*)"))
@angelbot.on(sudo_cmd(pattern="ikuck ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if event.is_private:
    ...
    q = 0
    r = 0
    angel = await edit_or_reply(event, "Searching Participant Lists.")
    async for i in bot.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await eod(angel, "I need admin priveleges to perform this action!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
...
Bots: {}
None: {}"""
        await angel.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
