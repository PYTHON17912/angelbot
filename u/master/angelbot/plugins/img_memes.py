...
@bot.on(angel_cmd(pattern=r"thug"))
...
async def _(event):
    ...
    angel = await eor(event, "`Converting To thug Image..`")
...
@bot.on(angel_cmd(pattern=r"trigger"))
...
@bot.on(angel_cmd(pattern=r"geyy"))
...
@bot.on(angel_cmd(pattern=r"pix"))
...
@bot.on(angel_cmd(pattern="ytc"))
@bot.on(sudo_cmd(pattern="ytc", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angel = await eor(event, "`Making a youthuub comment...`")
    owo = await event.get_reply_message()
