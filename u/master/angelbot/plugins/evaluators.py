...
async def _(event):
    ...
    angelevent = await eor(event, "`Executing.....`")
    process = await asyncio.create_subprocess_sangel(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    angeluser = await event.client.get_me()
    if angeluser.username:
        curruser = angeluser.username
    else:
        curruser = "angelbot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    angelevent = await eor(event, "`Running ...`")
    ...
    if exc:
        ...
        evaluation = "Success"
    final_output = f"•  Eval : \n`{cmd}` \n\n•  Result : \n`{evaluation}` \n"
    await eor(angelevent, "**Eval Command Executed. Check out LOGGER for result.**")
    await event.client.send_message(
        lg_id,
...
@bot.on(angel_cmd(pattern="bash ?(.*)", outgoing=True))
...
CmdHelp("evaluators").add_command(
  ...
  "exec", "<command>", "Execute a Terminal command on angelBot server and shows details"
).add_command(
