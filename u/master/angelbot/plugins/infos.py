...
@bot.on(angel_cmd(pattern="recognize ?(.*)", outgoing=True))
...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    angel = await eor(event, "recognizeing this media")
    async with event.client.conversation(chat) as conv:
        ...
        if second.text.startswith("See next message."):
            ...
            angel = third.message.message
            await eor(event, angel)
...
@bot.on(angel_cmd(pattern="info ?(.*)", outgoing=True))
...
<b>┣𝙶𝚁𝙾𝚄𝙿 𝙸𝙽 𝙲𝙾𝙼𝙼𝙰𝙽</b>: {}
<b>┗━━━━━━━━━━━━━━━━━━━━</b>
<b> <a href='https://t.me/angel_BOT_SUPPORT'>𝙵𝚁𝙾𝙼 𝙳𝙰𝚃𝙰𝙱𝙰𝚂𝙴 𝙾𝙵 𝙳3𝚅𝙸𝙻 𝙱𝙾𝚃</a>  </b>
""".format(
        user_id,
        user_id,
        first_name,
...
@bot.on(angel_cmd(pattern="chatinfo(?: |$)(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="chatinfo(?: |$)(.*)", allow_sudo=True))
async def info(event):
    if event.fwd_from:
        return
    angel = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await angel.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(angel, "`An unexpected error has occurred.`")
    return
...
@bot.on(angel_cmd(pattern=r"users ?(.*)", outgoing=True))
...
@bot.on(angel_cmd(pattern="admins ?(.*)"))
...
@bot.on(angel_cmd(pattern="id$"))
@bot.on(sudo_cmd(pattern="id$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angel = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await angel.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
