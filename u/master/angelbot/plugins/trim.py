...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./trim/angelbot.media.ffmpeg"
...
@bot.on(angel_cmd(pattern="tsave$"))
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        ...
        if reply_message:
            ...
            angelevent = await eor(event, "`Saving the file...`")
            try:
                c_time = time.time()
                downloaded_file_name = await event.client.download_media(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, angelevent, c_time, "trying to download")
                    ),
                )
...
@bot.on(angel_cmd(pattern="vtrim"))
...
async def ff_mpeg_trim_cmd(event):
    ...
    angelevent = await eor(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await eod(
                angelevent, f"**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            await event.client.send_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, angelevent, c_time, "trying to upload")
                ),
            )
            os.remove(o)
        except Exception as e:
            return await eod(angelevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        ...
        await eod(angelevent, "RTFM")
        return
    end = datetime.datetime.now()
    ms = (end - start).seconds
    await eod(angelevent, f"`Completed Process in {ms} seconds`", 3)


@bot.on(angel_cmd(pattern="atrim"))
...
@bot.on(angel_cmd(pattern="tclear$"))
