...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        ammoca_message = ""
        angelkrish = await event.client.get_entity(event.sender_id)
        if angelkrish.bot or angelkrish.verified or angelkrish.support:
            return
        angelkrishm = f"[{get_display_name(angelkrish)}](tg://user?id={angelkrish.id})"
...
@bot.on(angel_cmd(pattern=r"tagall (.*)", outgoing=True))
