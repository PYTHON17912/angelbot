...
from . import *
from angelbot.sql.mute_sql import is_muted, mute, unmute
...
@bot.on(angel_cmd(pattern="setgpic$"))
...
@bot.on(angel_cmd(pattern="promote(?: |$)(.*)"))
...
async def promote(promt):
    ...
    angelevent = await eor(promt, "`Promoting User...`")
    ...
    try:
        ...
        await angelevent.edit(f"**🔥 Promoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{promt.chat.title}`!! \n**Admin Tag :**  `{rank}`")
    except BadRequestError:
        await angelevent.edit(NO_PERM)
        return
...
@bot.on(angel_cmd(pattern="demote(?: |$)(.*)"))
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    angelevent = await eor(dmod, "`Demoting User...`")
    rank = "ǟɖʍɨռ"
    ...
    try:
        ...
        await angelevent.edit(NO_PERM)
        return
    await angelevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{dmod.chat.title}`")
...
@bot.on(angel_cmd(pattern=r"mute ?(.*)"))
@bot.on(sudo_cmd(pattern=r"mute ?(.*)", allow_sudo=True))
async def muth(angel):
    if angel.is_private:
        await eor(angel, "**Enough of your bullshit  !!**")
        await sleep(2)
        await angel.get_reply_message()
        replied_user = await angel.client(GetFullUserRequest(angel.chat_id))
        if is_muted(angel.chat_id, angel.chat_id):
            return await angel.edit(
                "Nigga is already muted here 🥴"
            )
        if angel.chat_id == d3krish:
            return await eod(angel, "Nashe me hai kya lawde 🥴")
        try:
            mute(angel.chat_id, angel.chat_id)
        except Exception as e:
            await eor(angel, f"**Error **\n`{str(e)}`")
        else:
            await eor(angel, "**Chup Reh Lawde 🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
        angelevent = await eor(angel, "`Muting...`")
        input_str = angel.pattern_match.group(1)
        chat = await angel.get_chat()
        if angel.reply_to_msg_id:
            userid = (await angel.get_reply_message()).sender_id
            name = (await angel.client.get_entity(userid)).first_name
        elif input_str:
            if input_str.isdigit():
                try:
                    ...
                    return await angelevent.edit(str(ve))
            else:
                userid = (await angel.client.get_entity(input_str)).id
                name = (await angel.client.get_entity(userid)).first_name
        else:
            return await eod(angelevent, "I Need a user to mute!!", 5)
        if userid == d3krish:
            return await eod(angelevent, "Nashe me hai kya lawde", 5)
        if str(userid) in DEVLIST:
            return await eod(angelevent, "**Error Muting God**", 7)
        try:
            await angel.client.edit_permissions(
                chat.id,
                userid,
                until_date=None,
                send_messages=False,
...
@bot.on(angel_cmd(pattern=r"unmute ?(.*)"))
...
async def nomuth(evn):
    if evn.is_private:
        ...
        angelevent = await eor(evn, "`Unmuting...`")
        ...
        if evn.reply_to_msg_id:
            ...
            return await eod(angelevent, "I need a user to unmute!!", 3)
        try:
...
@bot.on(angel_cmd(pattern="ban(?: |$)(.*)"))
@bot.on(sudo_cmd(pattern="ban(?: |$)(.*)", allow_sudo=True))
@errors_handler
async def ban(bon):
    if bon.fwd_from:
        return
    angelevent = await eor(bon, "`Banning Nigga...`")
    ...
    if not user:
        return await angelevent.edit("`Reply to a user or give username!!`")
    if str(user.id) in DEVLIST:
        return await angelevent.edit("**Say again? Ban my creator??**")
    try:
    ...
    try:
        ...
        await angelevent.edit(f"**Banned  [{user.first_name}](tg://user?id={user.id})  in** `[{bon.chat.title}]` !!\n\nMessage Nuking : **False**")
        return
    if reason:
        await angelevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]` !!\n**Reason :** `{reason}`")
    else:
        await angelevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]`!!")
...
@bot.on(angel_cmd(pattern="unban(?: |$)(.*)"))
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    angelevent = await eor(unbon, "`Unbanning...`")
    ...
    try:
        ...
        await angelevent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Is Now Unbanned in**  `{unbon.chat.title}` !!")
        ...
        await angelevent.edit("Invalid UserId!! Please Recheck it!!")


@bot.on(angel_cmd(pattern="pin($| (.*))"))
...
async def kick(usr):
    ...
    angelevent = await eor(usr, "`Kicking...`")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await angelevent.edit(NO_PERM + f"\n`{str(e)}`")
        return
    if reason:
        await angelevent.edit(
            f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`\nReason: `{reason}`"
        )
    else:
        await angelevent.edit(f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`")
...
@bot.on(angel_cmd(pattern=f"zombies ?(.*)"))
...
@bot.on(angel_cmd(pattern="undlt$"))
