...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        execl(executable, executable, "bash", "angelBot")


@bot.on(angel_cmd(pattern="restart$"))
@bot.on(sudo_cmd(pattern="restart$", allow_sudo=True))
async def re(angel):
    if angel.fwd_from:
        return
    event = await eor(angel, "Restarting Dynos ...")
    if HEROKU_API_KEY:
        await restart(event)
    else:
...
@bot.on(angel_cmd(pattern="shutdown$"))
@bot.on(sudo_cmd(pattern="shutdown$", allow_sudo=True))
async def down(angel):
    if angel.fwd_from:
        return
    await eor(angel, "**[ ! ]** Turning off 𝖣3𝗏𝗂𝗅𝖡𝗈𝗍 Dynos... Manually turn me on later ಠ_ಠ")
...
@bot.on(angel_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", outgoing=True))
@bot.on(sudo_cmd(pattern="(set|get|del) var(?: |$)(.*)(?: |$)([\s\S]*)", allow_sudo=True))
async def variable(angel):
    if angel.fwd_from:
        return
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await eor(angel, "`[HEROKU]:" "\nPlease setup your` **HEROKU_APP_NAME**")
    exe = angel.pattern_match.group(1)
    heroku_var = app.config()
    if exe == "get":
        event = await eor(angel, "Getting Variable Info...")
        await asyncio.sleep(1.5)
        cap = "Logger me chala jaa bsdk."
        capn = "Saved in LOGGER_ID !!"
        try:
            variable = angel.pattern_match.group(2).split()[0]
            if variable in ("angelBOT_SESSION", "BOT_TOKEN", "HEROKU_API_KEY"):
                if Config.ABUSE == "ON":
                    await bot.send_file(angel.chat_id, cjb, caption=cap)
                    await event.delete()
        ...
        event = await eor(angel, "Setting Heroku Variable...")
        variable = angel.pattern_match.group(2)
        if not variable:
            return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        value = angel.pattern_match.group(3)
        if not value:
            variable = variable.split()[0]
            try:
                value = angel.pattern_match.group(2).split()[1]
            except IndexError:
                return await event.edit(f"`{hl}set var <Var Name> <Value>`")
        await asyncio.sleep(1.5)
        if variable in heroku_var:
        ...
        event = await eor(angel, "Getting info to delete Variable")
        try:
            variable = angel.pattern_match.group(2).split()[0]
        except IndexError:
...
@bot.on(angel_cmd(pattern="usage(?: |$)", outgoing=True))
@bot.on(sudo_cmd(pattern="usage(?: |$)", allow_sudo=True))
async def dyno_usage(angel):
    if angel.fwd_from:
        return
    event = await edit_or_reply(angel, "`Processing...`")
    useragent = (
    ...
    return await event.edit(
        ...
        f"     ☆  `{hours}`**𝙷**  `{minutes}`**𝙼**  "
        f"**|**  `{percentage}`**%**"
        f"\n\n**𝙼𝙰𝚂𝚃𝙴𝚁 ➪ :** {angel_mention}"
    )


@bot.on(angel_cmd(pattern="logs$"))
@bot.on(sudo_cmd(pattern="logs$", allow_sudo=True))
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await eor(dyno, f"Make Sure Your HEROKU_APP_NAME & HEROKU_API_KEY are filled correct. Visit {angel_grp} for help.", link_preview=False)
    try:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        app = Heroku.app(HEROKU_APP_NAME)
    except BaseException:
        return await dyno.reply(f"Make Sure Your Heroku AppName & API Key are filled correct. Visit {angel_grp} for help.", link_preview=False)
    event = await eor(dyno, "Downloading Logs...")
    with open("angelbot-logs.txt", "w") as log:
    ...
    await event.delete()
    return os.remove("angelbot-logs.txt")
    
  # angel_data = app.get_log()
  # await eor(
  #     dyno, angel_data, deflink=True, linktext=f"**✔︎ 𝙷𝙴𝚁𝙾𝙺𝚄 𝙻𝙾𝙶𝚂 𝙾𝙵 💯 𝙻𝙸𝙽𝙴𝚂. 🗒️**\n\n **𝙼𝙰𝚂𝚃𝙴𝚁 ➪**  {angel_mention}\n\n🚀** 𝙿𝙰𝚂𝚃𝙴𝙳**  "
  # )
"""
    key = (
        requests.post("https://nekobin.com/api/documents", json={"content": angel_data})
        .json()
        .get("result")
        .get("key")
    )
    angel_url = f"https://nekobin.com/{key}"
    url_raw = f"https://nekobin.com/raw/{key}"
    foutput = f"**✔︎ 𝙷𝙴𝚁𝙾𝙺𝚄 𝙻𝙾𝙶𝚂 𝙾𝙵 💯 𝙻𝙸𝙽𝙴𝚂. 🗒️** \n\n [Nekobin]({angel_url}) & [Raw]({url_raw}) \n\n **𝙼𝙰𝚂𝚃𝙴𝚁 ➪**  {angel_mention}"
"""
