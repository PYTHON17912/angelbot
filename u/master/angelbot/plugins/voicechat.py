...
@bot.on(angel_cmd(pattern="startvc$"))
...
@bot.on(angel_cmd(pattern="endvc$"))
...
@bot.on(angel_cmd(pattern="vcinvite$"))
@bot.on(sudo_cmd(pattern="vcinvite$", allow_sudo=True))
async def _(event):
    angel = await eor(event, "`👥 Inviting Users To Join Voice Chat....`")
    users = []
    i = 0
    ...
    for k in d3_:
        try:
            ...
            pass
    await angel.edit(f"**👥 Invited {i} Users to  JoinVoice Chat**")
