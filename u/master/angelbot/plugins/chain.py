...
@angelbot.on(angel_cmd(pattern="chain$"))
@angelbot.on(sudo_cmd(pattern="chain$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    angel = await eor(event, "Counting...")
    count = -1
    ...
    while message:
        ...
        message = reply
        count += 1
    await angel.edit(f"⛓️ **Chain length :**  `{count}`")
