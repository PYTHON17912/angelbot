...
angel_NAME = Config.YOUR_NAME or "⋆𝗔𝘆𝘂𝘀𝗵𝗮™⋆"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await eor(event, 
                   "✓ **𝖥𝗂𝗅𝖾 𝗎𝗉𝗅𝗈𝖺𝖽𝖾𝖽 𝗍𝗈 : https://telegra.ph{}** \n✓ **𝖳𝗂𝗆𝖾 𝗍𝖺𝗄𝖾𝗇 :-** `{}` 𝗌𝖾𝖼𝗌 \n✓ **𝖡𝗒 :- {}**".format(
                        media_urls[0], (ms + ms_two), angel_mention,
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            ms = (end - start).seconds
            angelboy = f"https://telegra.ph/{response['path']}"
            await edit_or_reply(event, 
                  f"✓ **𝗉𝖺𝗌𝗍𝖾𝖽 𝗍𝗈** {angelboy} \n✓ **𝖳𝗂𝗆𝖾 𝗍𝖺𝗄𝖾𝗇 :-** `{ms}` 𝖲𝖾𝖼𝗌\n✓** 𝖡𝗒 :**  {angel_mention}", link_preview=True)
    else:
        await eod(event, 
