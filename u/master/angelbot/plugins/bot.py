...
@bot.on(angel_cmd("kickme", outgoing=True))
async def leave(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("😪 **I am leaving this group** See u all in `@im_angel_girl00`!!")
        time.sleep(1)
        if "-" in str(e.chat_id):
...
@bot.on(angel_cmd(pattern=r"config"))
...
@bot.on(angel_cmd(pattern="schd ?(.*)", outgoing=True))
