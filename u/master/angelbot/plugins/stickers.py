...
angelbot = Config.STICKER_PACKNAME


@bot.on(angel_cmd(outgoing=True, pattern="kang"))
...
async def kang(args):
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            angel = await eor(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
    ...
    if photo:
        ...
        packname = f"{user.username}_angelbot_{pack}"
        packnick = (
            f"{angelbot} Vol.{pack}"
            if angelbot
            else f"@{user.username}'s angelBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
        ...
        if (
            ...
            await angel.edit("`Preparing a new pack....`")
            async with bot.conversation("Stickers") as conv:
                await conv.send_message(cmd)
                await conv.get_response()
                # Ensure user doesn't get spamming notifications
...
@bot.on(angel_cmd(pattern=r"delst ?(.*)", outgoing=True))
...
@bot.on(angel_cmd(pattern=r"editst ?(.*)", outgoing=True))
    ...
    if angel == "":
        ...
        async with bot.conversation(chat) as conv:
            try:
                ...
                await conv.get_response()
                await asyncio.sleep(2)
                await conv.send_message(f"{angel}")
                response = await response
            except YouBlockedUserError:
                await event.reply("Please unblock @Stickers and try again")
                return
...
@bot.on(angel_cmd(pattern="pkang ?(.*)"))
...
async def _(event):
    ...
    un_ = user.username if user.username else angelkrish
    if not rply:
    ...
    if rply and rply.media and rply.media.document.mime_type == "image/webp":
        angel_id = rply.media.document.attributes[1].stickerset.id
        angel_hash = rply.media.document.attributes[1].stickerset.access_hash
        got_stcr = await bot(
            functions.messages.GetStickerSetRequest(
                stickerset=types.InputStickerSetID(id=angel_id, access_hash=angel_hash)
            )
        )
        stcrs = []
        ...
        try:
            create_st = await tbot(
                functions.stickers.CreateStickerSetRequest(
                    user_id=angelkrish,
                    title=pname,
                    short_name=f"angel_{un_}_pack{pack}_by_{bot_un}",
                    stickers=stcrs,
                )
            )
...
async def sticklet(event):
    ...
    image_stream.name = "angelbot.webp"
...
@bot.on(angel_cmd(pattern="waifu(?: |$)(.*)", outgoing=True))
