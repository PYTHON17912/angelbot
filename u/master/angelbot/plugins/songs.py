...
async def nope(angelkrish):
    ...
    await eor(angelkrish, f"Searching lyrics for  `{angel}` ...")
    if not angel:
        if angelkrish.is_reply:
            (await angelkrish.get_reply_message()).message
        else:
            await eod(angelkrish, "Give song name to get lyrics...")
            return
...
@bot.on(angel_cmd(pattern="song(?: |$)(.*)", outgoing=True))
...
@bot.on(angel_cmd(pattern="vsong(?: |$)(.*)", outgoing=True))
