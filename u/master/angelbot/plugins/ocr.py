...
@bot.on(angel_cmd(pattern="ocrlang", outgoing=True))
...
@bot.on(angel_cmd(pattern=r"ocr (.*)", outgoing=True))
...
async def parse_ocr_space_api(event):
    ...
    try:
        ...
        await eod(angel, "**Errors !!** \n`{}`\n**Report This to** {}\n\n`{}`".format(
                str(e), angel_grp, json.dumps(test_file, sort_keys=True, indent=4)
            )
        )
    else:
        await angel.edit("Read Document in {} seconds. \n{}".format(
                ProcessingTimeInMilliseconds, ParsedText
            )
        )
    os.remove(downloaded_file_name)
    await angel.edit(ParsedText)
