...
from angelbot.sql import sudo_sql as s_ql

# admin cmd or normal user cmd
def angel_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                angelreg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                angelreg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(angelreg + pattern)
    ...
    # blacklisted chats. 
    # angelbot will not respond in these chats.
