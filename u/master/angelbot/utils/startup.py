...
from angelbot import LOGS
from angelbot.config import Config
from angelbot.helpers.int_str import make_int
from angelbot.sql.gvar_sql import addgvar, gvarstat


# Creates the logger group on first deploy and adds the helper bot
async def logger_id(client):
    desc = "A Bot Logger Group For angelBot. DO NOT LEAVE THIS GROUP!!"
    ...
    try:
        grp = await client(functions.channels.CreateChannelRequest(title="angelbot Logger", about=desc, megagroup=True))
...
<b><i>»» <u><a href='https://t.me/im_angel_girl00'>
🎖ʟɛɢɛռɖǟʀʏ ǟʄ ⋆𝗔𝘆𝘂𝘀𝗵𝗮™⋆🎖</a></u> ««</i></b>
"""
    await client.send_file(
        ...
        caption=text,
        parse_mode="HTML",
        buttons=[[Button.url("angelBot Channel", "https://t.me/angel_BOT_OFFICIAL")]],
    )


# Joins the angelbot chat and channel from all clients
async def join_it(client):
    if client:
        try:
            await client(functions.channels.JoinChannelRequest("angel_BOT_SUPPORT"))
            await client(functions.channel.JoinChannelRequest('angel_BOT_OFFICIAL'))
